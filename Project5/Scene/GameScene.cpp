...
GameScene::GameScene()
{
	...
	for (int y = 0; y < ENYMY_CNT_Y; y++)
	{
		for (int x = 0; x < ENYMY_CNT_X; x++)
		{
			...
			tmpMoveState.emplace_back(MOVE_TYPE::WAIT, Vector2Dbl{ static_cast<double>((x + y * ENYMY_CNT_X) % 3) * 5.0 + static_cast<double>((x + y * ENYMY_CNT_X) / 3) * static_cast<double>(SPIRAL_CNT_MAX + SIGMOID_CNT_MAX + PIT_IN_CNT_MAX),0.0 });

			// 出てくる順番によって左右、上下位置を変更
			tmpMoveState.emplace_back(MOVE_TYPE::SIGMOID,
									  Vector2Dbl{ static_cast<double>((gameScreenSize.x * 2 / 7) 
																   + ((((x + y * ENYMY_CNT_X) / 3) + 1) % 2) 
																   * (gameScreenSize.x * 3 / 7)) ,
												  static_cast<double>((gameScreenSize.y * 5 / 6)
																   - ((((x + y * ENYMY_CNT_X) % 18) / 3) / 4) * SPIRAL_RADIUS * 2)
									  });
			...
			tmpMoveState.emplace_back(MOVE_TYPE::LR, 
									  Vector2Dbl{ static_cast<double>(2 - (x + y * ENYMY_CNT_X) % 3) * 5.0 + static_cast<double>((50 / 3) - (x + y * ENYMY_CNT_X) / 3) * static_cast<double>(SPIRAL_CNT_MAX + SIGMOID_CNT_MAX + PIT_IN_CNT_MAX),
												  0.0 
									  });
			...
			EnemyState eState = {
				ENEMY_TYPE::A,
				{ -15.0 + (static_cast<double>((((x + y * ENYMY_CNT_X)/ 3) % 2) * (gameScreenSize.x + static_cast<int>(cSize.x)))),
				   16.0 + (static_cast<double>((((x + y * ENYMY_CNT_X) % 18) / 3 / 2) * (gameScreenSize.y - 32 * 2) / 2))},
				{ 30, 32 } ,
				tmpMoveState
			};
